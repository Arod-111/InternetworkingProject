...
print("Welcome to the Regex and Dataset Testing Tool!")

while True:
    print("\nOptions:")
    print("1. Email Analysis")
    print("2. Domain Analysis")
    print("3. Test Against Phishing Domains")
    print("4. Test Against Phishing Links")
    print("5. Test Against Phishing Words")
    print("6. Test Against Custom Domains")
    print("7. Get Domain Report from VirusTotal")
    print("8. Get URL Report from VirusTotal")
    print("9. Test Against Phishing Attachments")
    print("10. Analysis a email")
    print("11. Quit")

    user_option = input("Please select an option (1-11): ")

    if user_option == '1':
        user_input = input("\nEnter Email Address: ")
        result1, result2 = analyze_email(user_input)
        
        if result1:
            print(f"\nResult: This may be a typo-squatting domain.")
        elif result2:
            print(f"\nResult: This may have double top-level domains.")
        else:
            print(f"\nDid not match, email is safe from typo-squatting and double top-level domains.")

    elif user_option == '2':
        user_input = input("\nEnter domain: ")
        result3 = analyze_domain(user_input)
        ...
        user_input = input("\nEnter text to test against phishing domains: ")
        result = test_phishing_domains(user_input)
        ...
        user_input = input("\nEnter text to test against phishing links: ")
        result = test_phishing_links(user_input)
        
        if result:
            print(f"\nResult: This may contain a phishing link.")
        else:
            print(f"\nNo match found, input is likely safe from phishing links.")

    elif user_option == '5':
        user_input = input("\nEnter text to test against phishing words: ")
        result = test_phishing_words(user_input)
        
        if result:
            print(f"\nResult: This may contain phishing-related words.")
        else:
            print(f"\nNo match found, input is likely safe from phishing words.")

    elif user_option == '6':
        user_input = input("\nEnter text to test against custom domains: ")
        result = test_similar_domains(user_input)
        
        if result:
            print(f"\nResult: This may contain a custom domain or similar domains:")
            for domain in result:
                print(domain)
        else:
            print(f"\nNo match found, input is likely not apart of the disposal domains.")
    
    elif user_option == '7':
        user_input = input("\nEnter domain to get report from VirusTotal: ")
        domain_report = get_domain_report(user_input)
        domain_info = extract_domain_info(domain_report)
        formatted_info = format_info(domain_info)
        print(formatted_info)
        
    elif user_option == '8':
        user_input = input("\nEnter url to virus total: ")
        url_id = encode_url(user_input)
        ...
        user_input = input("\nEnter attachment name to test against phishing attachments: ")
        result = test_phishing_attachments(user_input)
        ...
        for attachment in attachments:
            ...
            #Test attachments:
            attachment_test = test_phishing_words(attachment)
            if attachment_test:
                    print(f"\nResult: This may attachment type is used in phihsing attacks.")
            else:
                    print(f"\nNo match found, input is likely safe attachment.")    
